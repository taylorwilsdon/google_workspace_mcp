...
**Signed streaming attachment URLs** (stateless + OAuth 2.1 deployments): with `WORKSPACE_MCP_SIGNED_ATTACHMENT_URLS=true`, the download tools (`get_gmail_attachment_content`, `get_drive_file_download_url`) return a short-lived signed URL instead of base64-through-the-model or a file on disk. The download route recovers the owner's Google credentials and streams the bytes from Google on demand. URLs come in a short claim-check form by default (`/dl/{handle}`, ~60 chars, claims held in the shared KV store; `WORKSPACE_MCP_SHORT_SIGNED_URLS=false` to disable) with a self-contained signed-JWT fallback (`/attachments/signed/{token}`, needs no storage). `WORKSPACE_MCP_ATTACHMENT_SIGNING_KEY` sets the HMAC key, falling back to `GOOGLE_OAUTH_CLIENT_SECRET`. URL lifetime is clamped to the owner's token expiry, so a link can never outlive the credential that serves it.
