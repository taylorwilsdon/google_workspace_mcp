...
Signed download URLs (opt-in, for hosted deployments): with `WORKSPACE_MCP_SIGNED_DOWNLOAD_URLS=true`, `get_gmail_attachment_content`, `get_drive_file_download_url` and `get_gmail_message_content(full=true)` return a short-lived encrypted link (`/attachments/signed/{token}`) that streams the bytes from Google on demand, instead of writing the file to disk or inlining base64. The switch is off by default and needs `WORKSPACE_EXTERNAL_URL` as the links' public base plus key material (`GOOGLE_OAUTH_CLIENT_SECRET` or `FASTMCP_SERVER_AUTH_GOOGLE_JWT_SIGNING_KEY`) to derive the link key; with the switch on over streamable-http, the server refuses to start when either is missing, or in service-account mode (which keeps no per-user credentials to serve links with). A link lives up to 15 minutes, less when `WORKSPACE_MCP_OAUTH_PROXY_ACCESS_TOKEN_EXPIRY_SECONDS` is shorter; when no link can be issued the tool falls back to its normal delivery and says why. See the **[Advanced Deployment guide](https://workspacemcp.com/docs/deployment?utm_source=github.com&utm_medium=referral&utm_campaign=readme&utm_content=deploy-advanced)** for proxy, replica and logging setup.
