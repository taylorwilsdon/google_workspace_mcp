...
- **Restrict who the agent can reach.** For unattended deployments, set `WORKSPACE_ALLOWED_RECIPIENTS`: Gmail send/forward recipients, auto-forward filter targets, Calendar attendees, Drive access grants and ownership transfers are then checked in code before any API call and refused unless every address matches. Entries are exact addresses, whole-domain wildcards, or a mix — `WORKSPACE_ALLOWED_RECIPIENTS=*@mycompany.com,accountant@example.org` trusts your own workspace domain while still naming individual outsiders (`@mycompany.com` is accepted as shorthand for `*@mycompany.com`). Domain matching is exact, so `*@mycompany.com` does not cover `user@mail.mycompany.com`. `*` on its own lifts the restriction entirely; public "anyone with the link" sharing is blocked while the list is active unless `WORKSPACE_ALLOW_PUBLIC_SHARING=true` — including a Drive *domain* grant, even to an allowlisted domain, since exposing a file to everyone there is a different act from emailing one person. Unset means off.
