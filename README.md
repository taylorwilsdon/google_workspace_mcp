...
- **[OpenClaw managed HTTP deployment](docs/openclaw-deployment.md)** - opt-in `--preset openclaw` for one durable local Streamable HTTP process with coordinated credential refresh, instead of one stdio process per agent session
