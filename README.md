...
External OAuth provider mode checks every request's `ya29.*` token against Google's userinfo endpoint. Set `WORKSPACE_MCP_TOKEN_VALIDATION_CACHE_TTL` to a number of seconds to remember each validated token's identity for that long, so repeat calls with the same token skip the round-trip and do not occupy a validation worker. Only a hash of the token is kept, and failures are never cached. The trade-off is that a token revoked or expired within the TTL still passes this check (Google rejects it on the actual API call), so keep the TTL short. Unset or `0` disables the cache; invalid or negative values fail server startup.
