...
In legacy stdio mode the server opens the OAuth consent URL in the machine's default browser. Set `WORKSPACE_MCP_NO_BROWSER=true` to only return the URL in the tool result instead - for a server driven over SSH or a remote desktop, a headless box, or any setup where the consent is completed in a different browser profile than the one the desktop would open.
